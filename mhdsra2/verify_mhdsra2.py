...
from improved_dsra_mha import MHDSRA2Config, MultiHeadDSRA2, estimate_attention_memory_bytes, format_bytes
...
def scaling_report(args):
    ...
    print("Note: external exact token memory is assumed to live on CPU/NVMe; only retrieved tokens and landmarks are counted on GPU.")
...
def micro_benchmark(args):
    ...
    with torch.no_grad():
        for _ in range(args.steps):
            y, state = layer(x, state=state, retrieved_k=rk, retrieved_v=rv)
...
if __name__ == "__main__":
    ...
    p.add_argument("--seq-len", type=int, default=2_000_000)
    p.add_argument("--batch", type=int, default=1)
    p.add_argument("--dim", type=int, default=1024)
    p.add_argument("--heads", type=int, default=8)
    p.add_argument("--chunk", type=int, default=1024)
    p.add_argument("--slots", type=int, default=128)
    p.add_argument("--read-topk", type=int, default=8)
    p.add_argument("--write-topk", type=int, default=4)
    p.add_argument("--local-window", type=int, default=512)
    p.add_argument("--retrieval-tokens", type=int, default=128)
    p.add_argument("--dtype-bytes", type=int, default=2)
    p.add_argument("--page-size", type=int, default=1024)
    p.add_argument("--keep-full-io", action="store_true")
